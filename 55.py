...
def _is_palindrome(number):
    ...
    middle = length // 2
    first_part = number_str[:middle]
    last_part = number_str[-middle:][::-1]
...
def lychrel():
    ...
    for i in range(1, MAX_NUMBER + 1):
        ...
        is_palindrome = False
        iteration = 0
        while iteration < 50 and not is_palindrome:
            reversed_number = int(str(number)[::-1])
            number += reversed_number
            is_palindrome = _is_palindrome(number)
            iteration += 1
        if not is_palindrome:
            lychrel_numbers.append(i)
        is_palindrome = False
    print('There are {} lychrel numbers between 1 and {}'.format(
        len(lychrel_numbers), MAX_NUMBER
    ))
